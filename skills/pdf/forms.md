...
**CRITICAL: For PDF forms with CJK (Chinese/Japanese/Korean) text, you MUST use the provided `fill_pdf_form_with_annotations.py` script. Do NOT use pypdf's FreeText annotations directly - they will cause garbled text. The script automatically handles language detection and proper font embedding using ReportLab.**
